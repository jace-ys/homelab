...
resource "spacelift_stack_dependency" "cell_oci_root" {
  for_each = local.cells

  stack_id            = spacelift_stack.cell[each.key].id
  depends_on_stack_id = spacelift_stack.oci_root.id
}

resource "spacelift_stack_dependency_reference" "cell_compartment_ids" {
  for_each = local.cells

  stack_dependency_id = spacelift_stack_dependency.cell_oci_root[each.key].id
...
resource "spacelift_stack_dependency_reference" "cell_domain_name_external" {
  for_each = local.cells

  stack_dependency_id = spacelift_stack_dependency.cell_cloudflare[each.key].id
  output_name         = "${each.value.environment}_jaceystan_com"
  input_name          = "TF_VAR_domain_name_external"
}

resource "spacelift_stack_dependency_reference" "cell_domain_name_internal" {
  for_each = local.cells

  stack_dependency_id = spacelift_stack_dependency.cell_cloudflare[each.key].id
  output_name         = "homelab_${each.value.environment}_jaceystan_com"
  input_name          = "TF_VAR_domain_name_internal"
}
...
resource "spacelift_environment_variable" "ssh_public_key" {
