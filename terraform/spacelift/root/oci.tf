resource "spacelift_stack" "oci_root" {
  space_id                         = data.spacelift_space.root.id
  name                             = "oci-root"
  description                      = "☁️ OCI root"
  repository                       = "homelab"
  branch                           = "main"
...
resource "spacelift_stack_dependency" "oci_root" {
  stack_id            = spacelift_stack.oci_root.id
  depends_on_stack_id = data.spacelift_stack.spacelift_root.id
}

resource "spacelift_environment_variable" "oci_root_region" {
  stack_id   = spacelift_stack.oci_root.id
